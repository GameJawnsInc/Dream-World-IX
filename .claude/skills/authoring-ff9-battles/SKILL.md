...
description: Author custom FF9 battle content, zero DLL rebuild. Use for `battle-import`/`build`/`scene`/`list` (3D battle backgrounds -- reskin, loose-FBX, net-new scene, original `BBG_B###`), `battle-actions`/`ai`/`seq`/`patch`/`telemetry` (enemy+player tuning, AI `B_MEMBER` selectors, raw17 `btlseq` choreography, palette-swap enemies), CUSTOM SUMMONS/eidolons and their VFX (`summon-export`/`summon-rig-ref`/`summon-import`/`summon-deploy`/`summon-seq-lint`, `summon-reskin`/`summon-rescore` -- recolour or reframe a STOCK summon IN PLACE incl. `summon-rescore read`, the `[[summon]]` block, `.seq`/SFXRework casts, the s58 hybrid drive), battle LOCATIONS (`encounters` -- what is fought where, where a monster appears; `battle.locate`), or the `field.toml` blocks `[difficulty]`/`[rebalance]`/`[deathrules]` and `script`/`script.field`/`status` (the mod Scripts-DLL + Overload one-hub -- engine battle formulas). Also for a 9999 damage cap, a silently-dropped `.seq` op, or `[scene.ledger]` (a battle RECORDS the fight -- killer, hits, outcome -- into a field's persistent table). BattlePatch/DictionaryPatch/Scripts-DLL changes need a RELAUNCH. For a playable character's ability KIT see `creating-ff9-characters`.
---
...
- **The fight ledger:** `[scene.ledger]` (`declared_in` a field.toml + `table`, rows `on = init|reaction|dying`, `slot`, `cell`/`flag`, `set`/`add`) writes a field's `persist = true` table from the enemy AI -- tag 5 is the ATB turn, tag 7 the post-hit Reaction, tag 9 Dying needs `die_atk`. Reference: BATTLE_DESIGN.md § (b′); in-game record `studies/fight-ledger/PLAN.md`.
