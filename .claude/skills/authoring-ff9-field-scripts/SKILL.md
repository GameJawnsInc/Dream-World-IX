...
⚠ **Freshness caveat:** this surface landed 2026-07-24 and is still moving — the productized `[behavior]` TOML lane and the pooled-unit/wave rungs are playtest-proven, but the newest layer (data tables / counters / schedules on the `0xD3` VECTOR indexing dividend, bench 30415 "BTTABLE") is **deployed and playtest-PENDING**. Treat the docs as current-but-unsettled; verify against `ff9mapkit/docs/BEHAVIOR.md` and the memory files rather than assuming a stable contract.
