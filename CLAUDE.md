...
- Overload gameplay surface — the **ONE-HUB architecture** (the engine registers 1 IOverload\* implementer per interface per DLL, last-wins → the kit emits a single regenerated hub; features = plain static classes, mutators-before-observers, a collision gate, GENERIC deploy stickiness) + declarative **`[difficulty]`** (flag-gateable enemy HP/attack/magic scaling, mod-global, F6-toggleable) — ★ IN-GAME PROVEN 2026-07-11 (×2 HP byte-exact via the telemetry roster: Fang 68→136, Goblin 33→66, players untouched; hub-hosted telemetry + mutator-order proven in the same capture); the `flag` gate variant still to playtest → [[project-ff9-overload-hooks]]
