...
donors (18,15)/(21,10)/(16,17); old baselines left as-is by user call). Next = RowInsert growth on a multi-cell
base (needs a REGION-aware cut_census — the per-block one doesn't sweep interior borders) / rot+shift region
carry in-game check / z-axis RowInsert (the other growth axis) / band-crossing re-Wang / cliff-face transition
study (user checklist item); scale to a real CONTINENT (towns+entrances via `world-entrance --building`).
