...
**Frontier:** beach-mint rung 3 ★ PROVEN on (9,17); the REAL-SCALE island-B mint: beach ★ approved, cliffs FIXED at HEAD (the per-COLUMN lip anchor + the V-IN-BAND gate, ⏳ awaits the playtest — then merge to master); the OVERWORLD now deploys to the dedicated **`FF9CustomMap-world`** folder (campaign wholesale-replaces kept wiping FF9CustomMap's WorldMap tree) → [[project-ff9-handoff-islandb-cliff-lip]] → [[project-ff9-overworld-coast-mosaic]]; declarative `[[platform]]`; the story-event director (#13) — authoring side proven → [[project-ff9-fork-fidelity-worklist]].
