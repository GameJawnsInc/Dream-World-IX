...
- Path D, a genuinely new 3rd overworld world — ★★ rungs 0-5a; THE FULL SKIRT: the VISUAL junction PASSED (branch `claude/path-d-level-host`). Walkability DECODED + SIMULATED: the walk-query decode (double-verified) + the calibrated bench sim reproduce the sunken pin at 0.00u — the shingle cut missed ~half the covered lawn (sieve boundary vs whole-tri rule); next = the per-tri lawn-clip fix round, gated by walk_sim.py → `studies/path-d-new-world/WALK-QUERY-DECODE.md` + `BENCH-WALK-SIM.md`; read `GROUND-JUNCTION-SYNTHESIS.md` before wall/ground work
