...
**DECISION POINT (asked the user):** (A) accept the functional cutscene (move/turn/teleport/dialogue all work; animated walk-cycle + emotes are a known NPC limitation) and move on; or (B) pursue the animated-NPC fix — likely rebuild NPCs from a real animating NPC template (e.g. clone a real field's NPC entry) instead of the player object; bigger, iterative (F6 now makes it fast), uncertain. Quick diagnostic that narrows it: does a STANDING Vivi visibly idle-animate (breathe), or is he a frozen statue? (whole-animator-dead vs only-triggered-clips-missing). Engine F6 hotkey + non-locking cutscene are committed; CUTSCENE2 (4003, once=false) deployed.
