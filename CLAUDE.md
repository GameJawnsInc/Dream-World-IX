...
**AWAITING PLAYTEST 3.** Reach 4003 (New Game → Alexandria → hut → interior door → CUTSCENE2). Now: control locks, ~1s beat, Vivi **walks forward** to the green cross **without circling**, **turns to face you**, says "...hi.", **turns around and walks back** to his spot (the about-face is the fix in action), control returns. Then: talk → "Oh! You're finally here…"; leave + re-enter → **he just stands** (plays once). If he still won't reach a target, note which leg (forward vs the about-face). If it works, **animation + teleport walk-in** are the quick follow-ups.
