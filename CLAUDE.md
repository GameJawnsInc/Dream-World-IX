...
fact about the donor, not a mechanics gap); the −352 z-slide (island 4u south whole + the transposed boundary
fill at the empty corner's SOUTH border) deployed at (0,4)+2×3. Next = the topo-17/38/49 highland fill
vocabulary (hard) / band-crossing re-Wang / cliff-face transition study (user checklist item); scale to a real
CONTINENT (towns+entrances via `world-entrance --building`).
