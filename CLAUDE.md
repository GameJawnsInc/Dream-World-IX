...
**AWAITING PLAYTEST.** **Relaunch** (loads the new .eb), go to CUTSCENE2 (4003): Vivi should now **walk with his legs moving (no glide)** and **play the talk-gesture emote**, then say "...hi." + teleport home. F6 replays. Report: does he animate now? (I'll also confirm via `ANIMPROBE2`: `state=1` + `animFrame` advancing during the walk = fixed.)
