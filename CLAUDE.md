...
- Path D, a genuinely new 3rd overworld world — ★★ rungs 0-5a; THE FULL SKIRT visual junction PASSED; walkability FIXED (THE RENDER-ONLY UNDERLAY: 4078 walk-invisible tag, C-slice hem, owner-confirmed Z-fight gone) + THE CAMERA ORDER (ride-up decoded ff9.cs:2926 — tag-proof sky-cast; carried-first buffer, gate 100%) — all on `claude/path-d-level-host`, 5/5 gates. OPEN: the V-shore hover (wall over descending shore, measured; the CURTAIN mint registered, needs its study) → `studies/path-d-new-world/LAWN-CLIP-PREDICTION.md` + `WALK-QUERY-DECODE.md` + `BENCH-WALK-SIM.md`; read `GROUND-JUNCTION-SYNTHESIS.md` before wall/ground work
