...
m.obj]`** (`world/entrance.py`: clone+patch the `Byte[39]` trigger func → deploy to every dispatcher carrying the case ×7
langs, stacking + idempotent → tile bits → optional modelled building; offline-verified, ⚠ awaiting an in-game playtest).
