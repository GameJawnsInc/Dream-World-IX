...
**Next options:** Blender Tier-2 **Phase 3** (finalize add-on docs/version/repackage); a 2nd connected room (chain via gateway) to demo the multi-room approach; in-room **scrolling** or **multi-camera** as a new kit feature; open the upstream PRs (needs the user's GitHub fork + go-ahead); or the local engine→stock restore + a true-player-experience playthrough when desired.
