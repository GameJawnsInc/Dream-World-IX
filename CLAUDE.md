...
corrected line is **−332** (not −352); the island was REDEPLOYED with both fixes — confirmed live via tri-count +
mtime forensics. ★ In-game re-confirmation of the fix is the immediate next step. Then = the topo-17/38/49
highland fill vocabulary (hard) / band-crossing re-Wang / cliff-face transition study (user checklist item); scale
to a real CONTINENT (towns+entrances via `world-entrance --building`).
