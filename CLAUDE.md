...
**AWAITING PLAYTEST.** F6 reloads the field (test it — escapes locks, replays the scene). For CUTSCENE2 (4003): it should now run **without locking** — Vivi walks in (likely still gliding), snaps to face front (`turn=0`, instant — should be visible), plays the emote (does `RunAnimation 7302` show? — KEY diagnostic), says "...hi.", teleports home. **Please report: (a) does F6 reload the field? (b) does the instant turn (turn=0) visibly face him front? (c) does the emote animation play at all?** (b)+(c) tell us whether ANY non-idle anim works on this NPC, which decides the glide fix.
