...
The whole entrance flow is now FOLDED into one command — **`ff9mapkit world-entrance --cell X Z --field N [--building
m.obj]`** (`world/entrance.py`: clone+patch the `Byte[39]` trigger func → deploy per-language to every dispatcher carrying
the case, stacking + idempotent → tile bits → optional modelled building; **★ in-game proven 2026-07-01** — a Blender
castle spawned assembled+grounded, `!` → forked Ice Cavern). Note: patch each lang's OWN world .eb (JP dialogue differs);
seat buildings (don't `--flatten-pad`) — flatten reshapes walkable ground into stuck-steps unless kept under the building.
Remaining overworld frontier: texturing new geometry (atlas UVs); true new-continent mint (Path D).
