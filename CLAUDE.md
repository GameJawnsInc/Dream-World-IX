...
must run LAST + be re-run after any campaign re-deploy (else the link is wiped; symptom = landing on the real id). Remaining
= the elided **world-map leg** (overworld-seam boundaries have no `Field()` to retarget → flagged; needs a region-injected
warp). Before this: `deploy_campaign` productionized (auto-promote start-state CSVs to
