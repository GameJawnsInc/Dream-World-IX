...
COMPONENT, cut around, never through); net: (9,5)+2×3 has ZERO usable x-growth lines. Next = multi-boundary seam
extrusion (kill gap-vacation → the flat WATER lines open = whole-island slide growth in proven sea4; the cheap
unlock) / z-axis RowInsert / the topo-17/38/49 highland fill vocabulary (hard) / band-crossing re-Wang /
cliff-face transition study (user checklist item); scale to a real CONTINENT (towns+entrances via
`world-entrance --building`).
