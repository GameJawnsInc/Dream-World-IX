...
coastline moves — ★ the FIRST VISIBLE in-game bow proven 2026-07-08 (`VertexDisplace` tweak class + an A/B twin-island
harness; salience needs multi-column extent + a reference; fold/envelope/smoothed-normal laws → memory); next = bow v2 /
band-crossing re-Wang / column insertion; scale to a real CONTINENT (towns+entrances via `world-entrance --building`).
