...
- **THE TOPOGRAPH 36-38 ENCOUNTER LAW** — falsified in-game TWICE. `case 205`/`w_frameEventBattleProb` is the
  ...
  table hole, not a topograph gate. `world-encounter-rate` is a misnomer — the real lever is `world-encounter-frequency`.
...
- Co-op field/dialogue lockstep (F3) — ★ solo benches PROVEN unattended, captured s84; F3.1 talk relay solo-proven (s85, v13); two-machine pending → `studies/field-coop/F1-BUILD.md`
...
- Story-write trace (s88) — ★★ rungs 0-2 in-game PROVEN: which script wrote each story byte; the null pair (552 vs its verbatim fork) diffs empty; next = Dali → `studies/story-trace/PLAN.md`
