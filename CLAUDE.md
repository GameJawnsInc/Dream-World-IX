...
- Active Time Events — full teardown + the `[ate]` authoring primitive (synthesize prompt + winATE menu + `GetChoose` branch on a CUSTOM field) AND cold-reproduce a REAL ATE in a `--verbatim` fork by seeding the ATE-availability WORD (`[startup] words = [{byte=236, value=…}]`). ★ BOTH IN-GAME PROVEN (synth menu @30007; real Small-Town Knight ATE @30006). Key: ATE arming gates on a story-set avail word (byte 236), NOT the scenario; menu entry is a tag-1 poll loop (`Wait(1)`+`op_01` JMP); each avail-word bit = one menu row → [[project-ff9-ate-system]]
