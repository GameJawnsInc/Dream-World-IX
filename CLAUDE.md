...
**AWAITING PLAYTEST 4.** Reach 4003 (New Game → Alexandria → hut → interior door → CUTSCENE2). Now: control locks, ~1s beat, Vivi **walks forward** to greet you, **faces you**, says "...hi.", then **spins ~180° and walks back** to his spot (a quick spin-and-go via the high turn-speed, not a slow pivot), control returns. Then: talk → "Oh! You're finally here…"; leave + re-enter → **he just stands** (plays once). If the walk-back still fails, note exactly what Vivi does. If it works, **animation + teleport walk-in** are the quick follow-ups.
