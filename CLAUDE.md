...
- Path D, a genuinely new 3rd overworld world — ★★ rungs 0-5a; THE FULL SKIRT: the VISUAL junction PASSED (branch `claude/path-d-level-host`). Walkability decoded, simulated, and FIXED offline: THE RENDER-ONLY UNDERLAY (engine mapid 4078 = walk-invisible; lawn ships continuous, covered pieces re-tagged) deployed at gates-green on BOTH suites (build + walk_sim 4/4) — awaiting the owner's walk playtest → `studies/path-d-new-world/LAWN-CLIP-PREDICTION.md` + `WALK-QUERY-DECODE.md` + `BENCH-WALK-SIM.md`; read `GROUND-JUNCTION-SYNTHESIS.md` before wall/ground work
