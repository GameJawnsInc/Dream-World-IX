...
**Scope now:** the importer handles **both simple AND complex multi-floor fields** — the "simple fields only" caveat is lifted (frame is exact for any field; in-game content-on-floor proven on GLGV, multi-floor in-game re-test pending but the frame fits the header box). Remaining: (c) editable-art v1b; an in-game pass on a multi-floor fork; and the kit's `extract`/`build` docs could note the per-floor write side for the exporter.
