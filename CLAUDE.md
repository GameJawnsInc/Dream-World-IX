...
**Open with the user:** circle back to the walkmesh adjustment + paint a real wide background for their room (wide guide is in `…/debug_proj/scroll_test/paint_*_WIDE.png`). Then **multi-camera** is the last planned camera-movement feature. Field 4003 currently holds the user's MY_ROOM (walkmesh + checker); revert via `py tools/scroll_out/revert_user_scroll.py`.
