...
→ [[project-ff9-overworld-terrain-authoring]]. Custom-SHAPED coastlines: `world-coast-custom` ribbon-warps a real beach
onto an arbitrary shoreline curve (backward-resample; offline-built + review-hardened 2026-07-06, ⏳ awaits playtest →
[[project-ff9-overworld-coast-mosaic]]). Remaining overworld frontier: warp the near-shore foam/shallow GRADIENT to hug
the bent shore (v1 is a flat deep plane); scale to a real CONTINENT (towns+entrances via `world-entrance --building`);
texture new geometry (atlas UVs).
