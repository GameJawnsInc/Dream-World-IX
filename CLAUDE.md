...
- Path D, a genuinely new 3rd overworld world — ★★ rungs 0-5a; THE FULL SKIRT: the VISUAL junction PASSED (overlay + shingle cut, branch `claude/path-d-level-host`). ⚠ open front = WALKABILITY of multi-surface ground (sunken spots, missed hills) — next: the walk-query decode + bench walk simulator → `studies/path-d-new-world/FULL-SKIRT-PREDICTION.md`; read `GROUND-JUNCTION-SYNTHESIS.md` before wall/ground work
