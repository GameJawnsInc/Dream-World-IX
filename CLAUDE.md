...
residual verbatim). Shore vocabulary now lacks only the SAND band + cap-assembly synthesis — the rungs
before BEACH-MINT. The SAND STUDY is BANKED (falsified: wallpaper both frames, continuous ribbon;
confirmed: rect-lattice pinning + shared edge values + mirror-local repeats = the THIRD discrete
edge-matched language) — **next session = Path B: growth-only local mirror continuation** (extend each
column's own tile map, reflect at rect edges → beach_reshape v3 with a truly sliding seam = TRUE beach
growth past the ±2.5u drag cap; Path A = the full edge-table, the beach-mint prerequisite, later).
21 golden tests → [[project-ff9-overworld-coast-mosaic]].
...
kind; scan proven on all 3 forests, 35 slots each; popup+give+tally agree by construction) — offline-proven + deployed
in-place on 2950 (timer 120 + item slots → Elixir), **awaiting playtest** → [[project-ff9-chocobo-hot-cold]].
