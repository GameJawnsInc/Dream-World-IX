...
| GUI apps | in **`apps/`** — ONE app now (GUI makeover COMPLETE, Phase 6): `ff9_workspace.pyw` (= `ff9_studio.pyw`, the front door) launches the **PySide6 Workspace** — one dockable window: a **journey ▸ campaign ▸ field ▸ object** tree + breadcrumb, and as tabs the **Editor** (field/NPC/gateway/event/marker/cutscene/choice forms + catalog picker), **Map** (campaign graph), **Story State** + **Item & Equip** save editors, **Build & Deploy** (field/campaign/battle, auto-detected), and **Import** (fork a real field; fork-report/read-dialogue/inspect-save), with **Info Hub** browse + a **Ctrl-K** palette + a bottom Output/Problems console; dialogue fields (NPC/event/choice) carry a **live FF9-window wrap preview**. The 8 standalone tkinter `.pyw` (campaign_editor/import/build_gui/editor/dialogue/infohub/storystate/items) were **RETIRED** — everything they did is folded into the Workspace over the same tk-free backends (`editor/{forms,model,theme,feedback,breadcrumb,jobs}`, `campaign`, `save`, `save_items`, `flags`, `infohub`, `dialogue`). PySide6 = optional `gui` extra. → [[project-ff9-gui-makeover]] |
