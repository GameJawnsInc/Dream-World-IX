...
**AWAITING PLAYTEST.** Reach 4003 (`once=false` now, so re-enter to replay). Vivi starts at the cyan HOME, walks in to the magenta spot **with his walk animation (no glide)**, faces you, emotes, says "...hi.", teleports home. Flag if he still glides (then the fix needs a different approach — RunAnimation(walk) explicitly). Re-test loop: out the back door + back in.
