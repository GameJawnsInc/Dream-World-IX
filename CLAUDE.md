...
**Research status:** the SHAPE dimension + top-actions (floor/gateway) + re-entry + flexible input are exhausted on GZML. Remaining catalogued items need a different field or are game-owned: **worldmap top** (built, not walked — world-map state is game-owned, may misbehave in the debug warp); **two-way mount** (#5) + **per-end dismount anims** (#3) need a floor-at-both-ends field (CPMP/TRNO), not GZML. Test fields: `tools/scroll_out/ladtest_gzml/GZML_{SLANT,INPUT,BENT}.field.toml`.

**Carry-over / state:** on `master`, 393 tests. Field 4003 = GZML_BENT (revert `tools/scroll_out/revert_deploy.py`). Debug New-Game warp field 70 → `Field(4003, entrance 11)`. Standing constraint: **nothing public.**
