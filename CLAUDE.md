...
- Path D, a genuinely new 3rd overworld world — ★★ rungs 0-5a; **V-SHORE CORNER ★ CLOSED** (owner-confirmed flow + look) and the bench is REPRODUCIBLE: rebuild/verify with `py studies/path-d-new-world/bench_pipeline.py all` — every bench generator now refuses to deploy a corner-less bench; next arcs → `NEXT-STUDIES.md`; laws + gates → [[project-ff9-overhang-context-law]]; read `GROUND-JUNCTION-SYNTHESIS.md` before wall/ground work
