...
- Active Time Events — full teardown + the `[ate]` authoring primitive (synthesize the prompt + winATE menu + `GetChoose` branch on a CUSTOM field, no carried logic); ★ IN-GAME PROVEN (prompt→SELECT→menu→pick). The menu entry is an explicit tag-1 poll loop (`Wait(1)`+`op_01` JMP); a row's `reply` = text, `warp` = the authentic hub→cutscene-field flow → [[project-ff9-ate-system]]
