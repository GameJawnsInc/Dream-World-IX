...
→ [[project-ff9-overworld-terrain-authoring]]. **FULLY-SYNTHETIC CLIFF ISLAND ★ in-game proven 2026-07-07** — organic coast +
rock wall + the real grass/meadow/relief tile language + the RE'd engine PLACEMENT spec, all offline-gated (`scratch/synth-island/`;
→ [[project-ff9-overworld-placement-rules]], [[project-ff9-overworld-coast-mosaic]]). Remaining overworld frontier: BEACH-type
shores (sand/foam ribbons are copy-only); multi-cell landmasses; scale to a real CONTINENT (towns+entrances via
`world-entrance --building`); productize the island builder into the kit.
