...
coastline moves — ★ the first VISIBLE + SMOOTH in-game bow proven 2026-07-08 (`VertexDisplace` tweak class, sin² zero-end-slope
bell + local normals + a turn-language gate, A/B twin-island harness; salience needs multi-column extent + a reference;
all laws → memory); next = profile+material co-move / column insertion / band-crossing re-Wang / multi-cell carry;
scale to a real CONTINENT (towns+entrances via `world-entrance --building`).
