...
**AWAITING PLAYTEST (final v2 check).** **Relaunch** (clean engine + new .eb), go to CUTSCENE2 (4003): Vivi walks in (animated), turns, emotes, says "...hi.", then **teleports back onto the cyan cross** (his home) — control returns. Confirm the teleport lands on the cyan cross now (not where Zidane can walk over him). If good, v2 cutscenes (walk + animation + turn + say + teleport, all animated + correctly placed) are DONE.
