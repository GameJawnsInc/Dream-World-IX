...
**A working cutscene test is now deployed to 4003 (TESTROOM):** New Game → Alexandria → hut exterior → interior → interior DOOR → a narration cutscene plays on entry. Revert with `py tools/scroll_out/revert_deploy.py`. This is the user's pending re-verify of the editor end-to-end.
