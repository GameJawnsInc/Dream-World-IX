...
def frame_floor(cam: _cam.Cam, *, back_canvas_y: float = 130.0, front_canvas_y: float = 420.0,
    ...
    Raises ValueError if a requested row genuinely has no world z at this camera, quoting the
    REASON :func:`cam.canvas_row_z` gives (row past the horizon, root behind the camera, ...) and
    the horizon row. It used to blame "above the horizon" for every miss, which was false whenever
    the miss came from the old solver's pole-spanning bisection instead: at pitch 15 that refused
    the whole default frame with a horizon 320 rows away from the row it named."""
    zb_f, why_b = _cam.canvas_row_z(cam, back_canvas_y)
    zf_f, why_f = _cam.canvas_row_z(cam, front_canvas_y)
    if zb_f is None or zf_f is None:
        bad, val, why = (("back", back_canvas_y, why_b) if zb_f is None
                         else ("front", front_canvas_y, why_f))
        raise ValueError(
            f"floor {bad} edge (canvas Y={val:g}) has no world z at this camera "
            f"(pitch {_cam.pitch_deg(cam):.1f} deg, horizon at canvas "
            f"Y~{_cam.horizon_canvas_y(cam):.0f}): {why}.")
