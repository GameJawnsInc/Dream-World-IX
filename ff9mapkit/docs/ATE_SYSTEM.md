...
### `ATE(mode)` arg semantics (confirmed values `{0,1,2,5,6}`; C# comment says "rest unknown")

| `mode` | Effect | Mask logic |
|---|---|---|
| `0` | off / hide | `mode==0` early-hide in `SetAIcon` |
| `1` | show **Blue** (new) | `(mode&3)!=2` |
| `2` or `6` | blink **Gray** (seen) | `(mode&3)==2` |
| `\|4` (e.g. `5`) | **force-show**, ignore user-control | `(mode&4)>0` gate |
...
- **No net-new ATE authoring.** No `ate()` opcode wrapper, no `winATE`/caption synthesizer, no `content/ate.py`,
  no ATE test. Breadcrumbs only: `eb/opcodes.py:323` notes the "field-100 ATE menu"
  `EnableDialogChoices(VAR_GenInt16_241 | 32768, 0)` (the `0x8000` high bit on the availability mask is
  ATE-menu-specific and still unexplained in the kit); `flags.py:180-184` documents the `AteCheck` save location.
...
**Smallest path, in order:**

1. **Research/template (no production code yet, per project rule):** template the exact field-206 hub sequence
   (`ATE(mode)` → SELECT poll → `WindowSync(win,64,textID)` → `op_05({op7A(9)…})` → `op_0B` table → per-branch
   playback). Bytes are offline-extractable via `ff9mapkit.extract` (mapNo→FBG via `_fieldtable.py`).
2. **Sketch a `[[ate]]` primitive:**
   - `ate(mode)` wrapper in `eb/opcodes.py` (`encode(0xD7, mode)`) — trivial; encoder + tables already support it.
   - `content/ate.py` emitting either flavor: `type="compulsory"` (bracket a cutscene with `ate(1)`/`ate(0)` + a
     GLOB once-flag, mirrors field 1901); `type="menu"` (`ate(5)` + a `winATE(64)` window reusing the kit's
     existing `0x7C` `EnableDialogChoices`/choice machinery + a `GetChoose`→`op_0B` branch table). Blue-vs-Gray =
     read a `[[flag]]` GLOB bit, pass `1` vs `2`.
