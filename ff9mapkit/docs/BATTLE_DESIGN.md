...
| Radius / Geo / Mot[6] / Mesh[2] | collision + **model + 6 anim ids** (re-skin to a stock monster) | **raw16 only** | `@28 u16`, `@30 i16`, `@32` 6×u16, `@44` 2×u16 | No | **done (MESH re-skin, ★ in-game proven)** — `[[scene.enemy]] model=`/`model_scene=` transplants a real donor enemy's model block (`reskin.py`); swaps the MESH, but **all** animations (idle AND attack) stay the target's, retargeted onto the new mesh (scene/type-bound, `AnimationFactory.cs:60`) → build WARNS. Proven: a Goblin→Fang re-skin showed the Fang mesh posed by the Goblin's anims |
