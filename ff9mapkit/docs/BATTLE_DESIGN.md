...
| Radius / Geo / Mot[6] / Mesh[2] | collision + **model + 6 anim ids** (re-skin to a stock monster) | **raw16 only** | `@28 u16`, `@30 i16`, `@32` 6×u16, `@44` 2×u16 | No | **done (BODY re-skin)** — `[[scene.enemy]] model=`/`model_scene=` transplants a real donor enemy's whole model block (`reskin.py`); idle/damage/death = new model, but ATTACK anim stays the target's (raw17-driven, not carried) → build WARNS |
