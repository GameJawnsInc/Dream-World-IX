...
flag_base   = 8712             # campaign flag band START (= campaign.FIRST_SAFE_FLAG; first bit clear of ALL
                               #   real-FF9 usage. WAS 8300 (hit the Mognet lock band 8376-8511), then 8512 (hit the read-mail payload bytes).)
...
- **Flags:** parameterize the allocators by a **per-field `flag_base`** so field B never overlaps field A. Field `i` owns `[flag_base + i*K, flag_base + (i+1)*K)`, and within its block the three categories sub-partition (cutscene `base+0`, events `base+1..+31`, choices `base+32..+63`). **[LANDED 2026-06-10]** `build._FlagAlloc` threads an optional per-member `flag_base` through `build_script`/`lint_logic` (default `None` = the historical 8000/8100/8200 constants, so single-field builds stay **byte-identical**); `campaign.build_campaign` assigns each member `flag_base + i*K`. The default `flag_base` is now **8712** (`campaign.FIRST_SAFE_FLAG`) — the old **8300** collided with real-FF9's Mognet lock band at bits **8376–8511** (long mislabelled the treasure-chest bitfield), and the interim **8512** sat on stock read-mail's payload bytes (bits **8512–8711**, whole-byte-written by ordinary play). `lint_campaign` now errors if any member block, or any explicit flag, lands in the Mognet band or past the choice scratch (bit 16320). **[UPDATED 1.0.0b18]** the single-field defaults (`base=None`) are no longer the historical 8000/8100/8200 constants either — they moved into the safe band (`flags.AUTO_*_BASE`: event 9100+ / cutscene 9200+ / choice 9300+ / on_entry 9400+ / ate 9500+, with authored-flag skip), because the old on_entry/ate band (8300+) sat inside the stock Mognet MAILBOX slot bytes (Byte[1024–1045]).
