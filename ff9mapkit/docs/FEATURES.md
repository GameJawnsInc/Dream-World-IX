...
| Computed prop motion (`[[prop]] motion`) — orbit, sine-eased shuttle, bob, spin, swing; exact per-tick path from `ff9mapkit motion`; novel fields | ✓ | [FORMAT](FORMAT.md) |
| Photo mode (`[photo]`) — the player pans the camera across the painting, hides characters/set pieces step by step, tints the view, and gets everything back with a camera that eases onto him; novel fields | ✓ | [FORMAT](FORMAT.md) |
...
| **Summon texel repaint in place** (`summon-reskin export-art` + `[[reskin.texel]]`) — the second lever on the same verb: rewrites the texel INDICES (shape/edge/silhouette), not the palette; indexed-PNG paint workflow with a UV-coverage overlay, composes onto the CLUT lane in one container | ◐ *(creature texture pages — the one hazard-free texel class: offline-proven, indexed round trip byte-identical 93/93 across the corpus, a composed proof artifact gates clean end to end; in-game cast pending. Scenery pages are explicitly deferred, named W6b)* | [SUMMONS](SUMMONS.md) |
