...
| `arrive` | *(worldmap only, optional)* `[x, z]` world coordinates — pin the world **arrival point deterministically** (the exit presets the engine's persisted-position vars; the actor re-grounds on arrival). The natural pair for a custom entrance: arrive at your own door. Without it, the player returns wherever the position mirror last recorded them. `arrive_face` (0–255, default 0=south) sets the facing. |
