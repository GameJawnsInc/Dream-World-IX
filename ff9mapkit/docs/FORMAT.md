...
| `shadow` | OPTIONAL blob shadow — the `[[npc]] shadow` semantics: absent/`true` = the stock shadow for the variant's model (every TBX chest casts in stock: 221 of 224; F0 = `(10, 4)`), `false` = none, `{ size, intensity }` = override. Ignored on a field that ships `[field] mapconfig`. |
