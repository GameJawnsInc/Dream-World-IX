...
| `menu_pos` | **opt-in** window placement for the Moogle's own windows (`[MPOS=x,y]`). `"stock"` uses FF9's own pair — the option list at `(20, 16)`, every sub-window at `(30, 26)` — or give `[x, y]` for all of them. Omitted (default) lets the engine place the window, which is what the kit has shipped and playtested. |
