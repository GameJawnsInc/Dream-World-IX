...
| `mapconfig` | | a donor field's **MapConfigData** (the MCF), shipped as `EVT_<name>.bytes` under any scene type: the per-model blob shadow + tint and the per-floor lights the engine applies to **every** actor -- player, `[[npc]]`s, props, and grafted `[[object]]`s alike. `import --native` / `--verbatim` / `--editable` write it (`mapconfig.bytes`, SE-derived: regenerate, never commit). With it, `[player]`/`[[npc]] shadow` are ignored (build warning). A walkmesh reshaped through `[walkmesh] obj` keeps its lights: the build re-keys them through the `o floor_<donor index>` names both exporters write. Delete the line to fall back to the kit's census shadows (player + `[[npc]]` only, no tint). |
