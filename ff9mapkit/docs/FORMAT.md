...
[`[walkmesh]`](#walkmesh) ·
[`[photo]`](#photo-optional--player-photo-mode)
...
| `mapconfig` | | a donor field's **MapConfigData** (the MCF), shipped as `EVT_<name>.bytes` under any scene type: the per-model blob shadow + tint and the per-floor lights the engine applies to **every** actor -- player, `[[npc]]`s, props, and grafted `[[object]]`s alike. `import --native` / `--verbatim` / `--editable` and a plain (BG-borrow) `import` write it (`mapconfig.bytes`, SE-derived: regenerate, never commit). With it, the MCF sets every actor's shadow size and darkness, so a `shadow` key's one lever is OFF: `shadow = false` (on the player, an `[[npc]]`, a `[[chest]]`, a `[[savepoint]]`, a `[[prop]]`) and a held or stock-dark `[[prop]]` get stock's `DisableShadow` instead, and a `{ size, intensity }` table is ignored (build warning). A walkmesh reshaped through `[walkmesh] obj` keeps its lights: the build re-keys them through the `o floor_<donor index>` names both exporters write. A BG-borrow always ships it verbatim: the engine runs it on the donor's own `.bgi`, so every floor index is the donor's. Delete the line to fall back to the kit's census shadows (player, `[[npc]]`s and casting set pieces; no tint, and nothing on a grafted `[[object]]`). |
...
| `shadow` | OPTIONAL blob shadow under the player. Absent/`true` = the **stock** shadow for the model it wears (after any `model` re-skin): the size + darkness the shipping fields' MapConfigData gives that model, census in `_shadowparams.py` (Zidane `(9, 4)`). `false` = none. `{ size = N, intensity = N }` overrides either (size 0–255, the quad is 14·size × 12·size units; intensity 0–15, stock is mostly 3–4). **Intensity stops at 15 because the engine's blob colour wraps after it.** The op carries `intensity << 3` in one byte, but `SetRenderer` draws the blob in colour `(amp × 2) & 0xFF`, so 16–31 would draw exactly as 0–15: 16 draws no shadow at all, and 31 draws the same as 15 (checked in-game: `studies/actor-shadow/intensity_wrap.py`). Validate refuses 16–31 and names the model's census value. The census itself keeps stock's own 16 on two accessory models (200, 488), which wraps the same way in stock, so an absent key still matches stock. Emitted as `SetShadowSize` + `SetShadowAmplifier`, the two engine calls the MCF would make — a kit field ships no MCF, which is why its actors had no shadow. On a field that ships `[field] mapconfig` its MCF shadows every actor itself, so `false` there is stock's `DisableShadow` (kept off after every jump and ladder climb: the engine turns a jumper's shadow back on when it lands, and the kit switches it off again at the end of the move -- during the landing animation it shows) and a `{ size, intensity }` table is ignored (build warning). |
...
| `shadow` | OPTIONAL blob shadow — same key as `[player] shadow`: absent/`true` = the stock shadow for this NPC's model, `false` = none, `{ size, intensity }` = override. Covers behavior units (they are `[[npc]]`s). |
...
### Window styling (style · window · actor · instant · speed · duration · window_pos · box · no_turbo · no_focus · polled)
...
- **`no_turbo`** — emit `[NTUR]`, so a **turbo-dialog session cannot auto-close this window**. FF9's
  ...
  **Defaults to `true` on a READOUT window** — one whose text renders a live value (`[NUMB=]`,
  ...
  `duration` and `no_focus` also stop the fan-out, but they do it by setting the engine's
  *button-inhibit* flag — which means the player can't dismiss the window either, and a **blocking**
  window nobody can dismiss hangs the script forever. Nothing is added when one of those is already
...
- **`polled`** (`[[choice.options]]` and `[ate]` options — the two lanes that emit the poll) — the
  ...
  *asynchronously*, held, watched for a real button press, and closed by the script, instead of the
  ...
  all: every tag that survives a latched F9 also blocks the player's own Confirm, and then the
  blocking window hangs.

  There are **two** turbo mechanisms, not one, and a polled page has to beat both. The first fans a
  synthesized confirm out to every open window (that's what `no_turbo` blocks). The second fires
  only when every open window is button-inhibited *and* one of them is in the bubble/transparent
  family *and* the script is polling for input — and it writes a confirm straight into the script's
  own input stream, so the poll reads a press nobody made. That second one is beaten by `[NTUR]`
  **or** by a window style outside that family, and the kit ships both guards:
  ...
  reply = """…"""
  polled = true
  style = "plain"      # 0/4/8/64/132 are safe; bubble/transparent (128/16/144/160) are refused
  ```
...
bob as reading when it moves the prop at least one field pixel and either its own up-and-down is
**faster on screen than the path's** at every point of the lap (so it can turn the prop around
anywhere), or it **out-travels** the path's own up-and-down -- judged on the part of the path each
camera actually shows. That rule is the kit's model; the one owner observation behind it is the
invisible cask below. Neither the phase nor the direction changes the verdict, and a slightly faster
or larger bob never reads worse. On a radius-300, 128-tick orbit at that
pitch the orbit's up-and-down peaks at about 1.5 field px a tick: ±60 every 256 ticks (0.1 px a tick)
could not be seen, ±60 every 32 ticks (0.8) does not read either, and ±120 every 32 (1.6) and ±150
every 32 (2.0) do. A sideways shuttle, a small orbit under a big bob, or a bob-only prop has little or no
...
## `[photo]` (optional — player photo mode)

A **photo mode** for the player: a button press stops the player, and the view pans across the painting on the
d-pad. The player can hide characters and set pieces one at a time, and lay a warm tint over the screen. The close
button puts everything back and eases the camera back onto the player. A bare `[photo]` turns it on with the
defaults below:

```toml
[photo]                               # minimal: Select opens, d-pad pans, R1 hides, L1 tints, Cancel closes
```

```toml
[photo]                               # everything
open_button  = "select"               # select l1 r1 l2 r2
close_button = "cancel"               # cancel select l1 r1 l2 r2 -- may equal open_button (one button toggles it)
hide_button  = "r1"                   # each press hides the next step of `hide`
grade_button = "l1"                   # toggles the tint
hide  = ["guard", "lamp", "player", "all"]
grade = true
```

| key | type | default | meaning |
|---|---|---|---|
| `open_button` | button name | `"select"` | opens photo mode: one of `select`, `l1`, `r1`, `l2`, `r2` |
| `close_button` | button name | `"cancel"` | closes it: `cancel`, `select`, `l1`, `r1`, `l2` or `r2`; the same button as `open_button` makes one button a toggle |
| `hide_button` | button name | `"r1"` | each press hides the next step of `hide`; a press after the last step does nothing |
| `grade_button` | button name | `"l1"` | turns the tint on and off |
| `hide` | list of names | `["player", "all"]` | what the hide button hides, one step per press: `"player"`, the `name` of an `[[npc]]` or a `[[prop]]` (a prop is matched by its `name` key, and a composite hides every part), and `"all"` (every character and set piece left, except chests), which must be last. At most 8 steps and 15 objects. `[]` turns hiding off |
| `grade` | bool | `true` | the tint (a held warm grade: white becomes (235, 171, 107)); `false` turns it off |
...
hidden, but the player cannot move anyway. `"all"` is the engine's own hide-everything. So every hide target must
exist, unchanged, for the whole visit: a hidden-until-flagged NPC does not.

**Rules.** `ff9mapkit lint` and the build refuse each of these with the same message:

- **Novel fields only.** A field with `[verbatim_eb]`, a donor (`[verbatim_eb] donor`, `[field] source_field`,
  `[field] borrow_field`), a `[camera] borrow`, `[field] bgs` or `[field] borrow_bg` is refused: a donor's own camera
  scripts, other cameras and widescreen width are invisible to the build. `lint-campaign` refuses a campaign member
  the manifest forks from a real field the same way, and `tools/deploy_field.py` refuses a live ForkDonorPatch row
  for the id.
- **Buttons.**
  - `start` pauses, `menu` opens the main menu, and the d-pad pans.
  - `confirm` and `special` would also talk to whoever the player faces in the same tick.
  - Held Cancel is the walk modifier, so it cannot open photo mode.
...
**What it never does.** It never writes a story flag or a field variable: its state is its own script's. It never
reads a button with the edge test that arms Memoria's dialogue turbo. It never switches the camera services off,
...
| `shadow` | OPTIONAL blob shadow under the save point's Moogle — the `[[npc]] shadow` semantics (absent/`true` = the census, the moogle's `(6, 2)`: 58 of 58 stock save moogles keep theirs; the act's own `DisableShadow`/`EnableShadow` hop pair, verbatim from the donor, hides it mid-air). `false` turns off the Moogle's AND the `barrel_pop` cask's (which otherwise casts, as stock's casks do); a `{ size, intensity }` table sizes the Moogle's only. The act's book + feather never cast (their donor Init disables it). On a field that ships `[field] mapconfig`, `false` is stock's `DisableShadow` on the Moogle and the cask, and the act's landings keep it off; a table is ignored. |
...
| `shadow` | OPTIONAL blob shadow — the `[[npc]] shadow` semantics: absent/`true` = the stock shadow for the variant's model (every TBX chest casts in stock: 221 of 224; F0 = `(10, 4)`), `false` = none, `{ size, intensity }` = override. On a field that ships `[field] mapconfig`, `false` is stock's `DisableShadow` and a table is ignored. |
...
loss_sfx = 1942                    # (optional) loss sting — rings ~1s CLEAR before the loss
                                   ...
                                   # holds the dispatch level while the cue rings)
