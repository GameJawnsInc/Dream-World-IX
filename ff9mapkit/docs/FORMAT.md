...
| `collision` | OPTIONAL, default `true`. `false` = a **walk-through** prop (`SetObjectFlags` show-only, both collide bits clear — the shipping render-only-object pattern): the player passes straight over/through it. For floor markers (a `[[coop]]` plate stone, a painted-circle stand-in) and dense scenery. Talk/dialogue still works. |
