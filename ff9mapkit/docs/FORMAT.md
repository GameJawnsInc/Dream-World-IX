...
| `pos` | `[x, z]`. |
| `face` | OPTIONAL standing facing (0..255; 0=south/toward the camera, 64=west, 128=north, 192=east — the same compass `[player]`/chest/prop `face` uses). Absent = 0. Rides the object's own `TurnInstant`, so the NPC is already turned on frame 0. **Head-tracking is separate**: most NPC rigs still turn their *head* toward the player when you walk up (a per-model mask), but the body keeps this facing. |
