...
def parse_tables(memoria_root: Path):
    ...
    return table, len(id_to_fbg), len(id_to_evt)
...
def main(argv=None) -> int:
    ...
    table, n_fbg, n_evt = parse_tables(Path(args.memoria))
    target = Path(__file__).with_name("_fieldtable.py")
    target.write_text(render(table), encoding="utf-8", newline="\n")
    print(f"wrote {target}  (eventIDToFBGID={n_fbg}, EventDB={n_evt}, joined field maps={len(table)})")
    return 0
