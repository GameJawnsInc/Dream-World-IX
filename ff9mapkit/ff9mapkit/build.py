...
def detaching_donor(project: FieldProject) -> "int | None":
    ...
    forked in place, or the real field whose scene a BG-borrow ships under ``borrow_bg``. ``import`` records a
    BG-borrow's donor as ``source_field``; ``borrow_bg`` still covers an older toml or a hand-written borrow,
    which a campaign member's ``plan.members`` row fires all the same."""
