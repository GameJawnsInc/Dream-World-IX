...
def _validate_story_writes(d: dict, label: str, names: dict, problems: list, *,
    ...
    (e.g. the ATE-availability mask at byte 236) that spans two bytes. ``<bytes_key>`` (also ``[startup]``)
    is a list of ``{byte = <0..2047>, value = <0..255>}`` -- a save-backed SINGLE-byte write (no neighbour
    clobber). Appends human-readable problems."""
    ...
    if bytes_key is not None:
        ...
        if not isinstance(bw, list):
            problems.append(f"{label} {bytes_key} must be a list of {{byte = <0..{_startup.BYTE_BYTE_MAX}>, "
                            f"value = <0..{_startup.BYTE_VALUE_MAX}>}}")
        else:
            for i, b in enumerate(bw):
                if not isinstance(b, dict) or "byte" not in b or "value" not in b:
                    problems.append(f"{label} {bytes_key} #{i} needs `byte` (a gEventGlobal byte offset) and "
                                    f"`value` (a single byte, 0..{_startup.BYTE_VALUE_MAX})")
                    continue
                idx, val = b["byte"], b["value"]
                if isinstance(idx, bool) or not isinstance(idx, int) or not (0 <= idx <= _startup.BYTE_BYTE_MAX):
                    problems.append(f"{label} {bytes_key} #{i} byte must be 0..{_startup.BYTE_BYTE_MAX} (got {idx!r})")
                elif idx == 0:
                    problems.append(f"{label} {bytes_key} #{i}: byte 0 is the ScenarioCounter -- use "
                                    f"`scenario = N` instead")
...
def _validate_savepoint_mognet(project, sp) -> list:
    ...
    _keys = {"name", "give", "accept", "received", "mognet_row", "accept_prompt", "accept_yes",
             "accept_no", "thanks", "give_prompt", "give_yes", "give_no", "give_line", "nothing",
             "erase", "status_none", "menu_prompt", "accept_row", "read_row", "cancel_row",
             "read_prompt", "arrive_line"}
    ...
    # `received` -- auto-arriving letters (the Read-mail list's second source). Every entry needs the
    # full render kit (variant/from/title/letter); the gates are optional ints.
    rec = mg.get("received", [])
    if not isinstance(rec, (list, tuple)):
        out.append(f"[savepoint.mognet] received must be a list of tables, got {rec!r}")
        rec = []
    rec_variants = []
    for it in rec:
    ...
    try:
        acc_vs, _ = _mognet.normalize_accept(acc if isinstance(acc, (list, tuple)) else [])
    except (ValueError, TypeError, KeyError):
        acc_vs = []
    seen = {}
    for src, v in [("accept", v) for v in acc_vs] + [("received", v) for v in rec_variants]:
        if v in seen:
            out.append(f"[savepoint.mognet] variant {v} is authored twice ({seen[v]} + {src}) -- one "
                       f"variant is one letter forever (its one-shot locks are save-global)")
        else:
            seen[v] = src
    if isinstance(give, dict) and give.get("variant") in rec_variants:
        out.append(f"[savepoint.mognet] give.variant {give['variant']} is also in received -- a letter "
                   f"cannot be both FROM and TO this moogle (their one-shot locks would collide)")
    for k in ("mognet_row", "accept_prompt", "accept_yes", "accept_no", "thanks", "give_prompt",
              "give_yes", "give_no", "give_line", "nothing", "erase", "status_none", "menu_prompt",
              "accept_row", "read_row", "cancel_row", "read_prompt", "arrive_line"):
...
def validate(project: FieldProject) -> list[str]:
    ...
    for la in project.raw.get("ladder", []):
        ...
        if "top" in la or "bottom" in la:           # EMULATED BIDIRECTIONAL (from-scratch, no real climb)
            ...
            continue
        if "arc_from" in la or "arc_to" in la:      # RETIRED 2026-07-17 (never in FORMAT.md, zero users found)
            problems.append("[[ladder]] arc_from/arc_to was retired: use navigable=true with bottom/top "
                            "(FF9's real held-d-pad climb) or top/bottom with a height "
                            "(the animated bidirectional hop-arc)")
            continue
        z = la.get("zone", [])
    ...
    if dif is not None:
        ...
        try:
            ...
            problems.append(str(e))
    reb = project.raw.get("rebalance")                    # [rebalance]: HP-damage multiplier via the Overload hub
    if reb is not None:
        from .battle import rebalance as _rebalance
        try:
            _rebalance.parse_table(reb, name_map=story_names)
        except _rebalance.RebalanceError as e:
            problems.append(str(e))
    dr = project.raw.get("deathrules")                    # [deathrules]: game-over rules via the Overload hub
    if dr is not None:
        from .battle import deathrules as _deathrules
        try:
    ...
    for sp in project.raw.get("savepoint", []):         # synthesized save point (press -> Menu(4,0))
        ...
        if sp.get("shop") is not None:
            sid = sp["shop"]
            if not isinstance(sid, int) or isinstance(sid, bool) or not 0 <= sid <= _shop.MAX_SHOP_ID:
                problems.append(f"[[savepoint]] shop must be a shop id 0..{_shop.MAX_SHOP_ID} (the Menu "
                                f"sub-id the Mogshop row opens), got {sid!r}")
            elif not any(int(x.get("id", -1)) == sid for x in project.raw.get("shop", [])):
                problems.append(f"[[savepoint]] shop = {sid} has no [[shop]] with that id in this field -- "
                                f"the Mogshop row would open an inventory the mod never ships")
        for k in ("party_min", "party_locked"):
            if k in sp and (not isinstance(sp[k], int) or isinstance(sp[k], bool)):
                problems.append(f"[[savepoint]] {k} must be an integer, got {sp[k]!r}")
        if "party_min" in sp and not 0 <= sp.get("party_min", 0) <= 4:
            problems.append(f"[[savepoint]] party_min must be 0..4 (the minimum party size), "
                            f"got {sp['party_min']!r}")
    ...
    for _ci, cs in enumerate(_cs_blocks):
        ...
        # keeps the plain "[cutscene]" label (identical messages); plural blocks are labelled by index.
        lbl = "[cutscene]" if len(_cs_blocks) == 1 else f"[cutscene] #{_ci}"
        ...
        _validate_story_writes(cs, lbl, story_names, problems,
                               scenario_key="set_scenario", flags_key="set_flags")
        ...
        if isinstance(rs, str):
            try:
                ...
                problems.append(f"{lbl} requires_scenario: {e}")
        elif rs is not None and (isinstance(rs, bool) or not isinstance(rs, int)
                                 or not (0 <= rs <= _startup.SCENARIO_MAX)):
            problems.append(f"{lbl} requires_scenario must be 0..{_startup.SCENARIO_MAX} or an area "
                            f"name (got {rs!r})")
        ...
        if cs.get("ate"):
            ...
            if not isinstance(m, int) or isinstance(m, bool) or not (0 <= m <= 255):
                problems.append(f"{lbl} ate_mode {m!r} must be an int 0..255 "
    ...
    if len(_cs_blocks) > 1:
        ...
        for _ci, cs in enumerate(_cs_blocks):
            ...
            if key in _gates:
                ...
                problems.append(f"[cutscene] #{_gates[key]} and #{_ci} have {what} -- a [[cutscene]] dispatch "
...
def lint_logic(project: FieldProject) -> list[str]:
    ...
    for ev in raw.get("event", []):
        ...
        if ev.get("once", True):                       # mirror build_script's auto-allocation exactly
            ...
            counter += 1
    for _ck, cs in enumerate(_cutscene.blocks(raw.get("cutscene"))):   # a cutscene also sets flags
        for step in cs.get("steps", []):                               # (set_flag steps + its own once-flag)
            if "set_flag" in step:
                settable.add(int(step["set_flag"][0])); explicit.add(int(step["set_flag"][0]))
        if cs.get("once", True):
            f = int(cs["flag"]) if "flag" in cs else _auto.cutscene(_ck)
    ...
    if any(_b.get("ate") for _b in _cutscene.blocks(raw.get("cutscene"))):
...
def lint_flag_bands(project: FieldProject) -> list[str]:
    ...
    for _ck, cs in enumerate(_cutscene.blocks(raw.get("cutscene"))):
...
def _lint_scripts_toolchain(project: FieldProject, out: list) -> None:
    ...
    if project.raw.get("rebalance") is not None:
...
def _lint_fork_walkmesh_ids(project: FieldProject, wmesh, warnings: list) -> None:
    ...
    (``donor_field_id``) nor a donor reference mesh (an ``--editable`` toml written before import recorded
    ``source_field`` has no id, but its sibling ``walkmesh.bgi`` and carried sidecars make it a fork all the same); no donor reference mesh
...
class _FlagAlloc:
    ...
    def cutscene(self, k: int = 0) -> int:
        ...
        if k == 0:
            return self.base
        raise BuildError(
            "a second (or later) [[cutscene]] block in a campaign member needs an explicit `flag = N` -- "
            "the per-member flag block reserves exactly ONE cutscene slot, so an auto once-flag would "
...
def build_script(project: FieldProject, lang: str, dialogue_txids: dict,
    ...
    def _cs_ate(cs):
        # compulsory / auto-advance ATE styling (the forced flavor, e.g. field 956 / the grey mode-6 fields):
        # bracket the body ATE(mode)..ATE(0) + caption its windows winATE. `ate=true` -> mode 6; `ate_mode`
    ...
    for _k, cs in enumerate(cs_blocks):
        ...
        cs_once_flag = None
        if cs.get("once", True):
            cs_once_flag = int(cs["flag"]) if "flag" in cs else _auto.cutscene(_k)
        steps = [_cutscene.compile_steps(cs["steps"], _cs_slices[_k], say_flags=cs_say_flags)]
        then_warp = int(cs["then_warp"]) if cs.get("then_warp") else None
        _, cs_gate, cs_end = _cutscene_story_bits(cs)      # the director gate + story advance (#13)
    ...
    if savepoints:
        ...
        def _mognet_body(mg, t):
            """The Mognet row's body -- the full letter act (guard + status box + arrivals +
            accept/read-mail/give/nothing, docs/SAVEPOINT.md). give_to_id and the read-mail row/arrival
            metadata were resolved against the install roster at text time; their absence means the
            corresponding lane was not authored."""
            from .content import mognet as _mognet
            give = None
            ...
            rows = [tuple(r) for r in (t.get("read_rows_meta") or [])]
            received = [tuple(r) for r in (t.get("received_meta") or [])]
            lv = set(acc_letters) | {r[0] for r in received}
            return _mognet.mognet_interaction_body(
                ...
                letter_txids={v: t[f"letter{v}"] for v in lv if f"letter{v}" in t},
    ...
    # jumped over; NOT zero-filled, which made the Init yield 48 ticks before DefinePlayerCharacter). See
    # content.npc.neutralize_player_audio_cruft.
    ...
    if _settle:
        if _entry_settle.is_auto(_settle):         # "auto" = compute the hold from the warp-in delta (rung 7)
            ...
            try:
                ...
                _n = 0
        _pre_settle = eb
        # on the template the hold sits before Main_Init's `set MAP159 = 1`, so the template's own
        # (entrylock-gated) handshake grants; elsewhere the settle closes with its own EnableMove, which must
        # not re-grant on an arrive-locked entrance (the on_entry hook owns that grant) -- gate it on the ids
...
def collect_text(project: FieldProject):
    ...
    for k, sp in enumerate(project.raw.get("savepoint", [])):
        ...
        _label = {"save": sp.get("save_row", _savepoint.DEFAULT_SAVE_ROW),
                  ...
                  "mognet": (mg or {}).get("mognet_row", _mognet.DEFAULT_MOGNET_ROW),
                  "shop": sp.get("shop_row", _savepoint.DEFAULT_SHOP_ROW),
                  "party": sp.get("party_row", _savepoint.DEFAULT_PARTY_ROW),
                  "cancel": sp.get("cancel_row", _savepoint.DEFAULT_CANCEL_ROW)}
        ...
        if mg and str(mg.get("name", "")):
            ...
            # READ-MAIL: titled accept letters + `received` auto-arrivals form the menu rows, in
            # authored order (row index is static, like every stock field's). Senders resolve through
            # the install roster + our own name; the row/arrival metadata rides savepoint_txids as
            # pass-through values (like give_to_id), so the injection needn't re-fetch the roster.
            _full = list(mognet_roster) + [self_name]
            _rows_meta, _titles = [], []
            ...
            if _rows_meta:
                ...
                # an unavailable lane's row vanishes from navigation (stock's SetupChooseMask law)
                _arow = str(mg.get("accept_row", _mognet.DEFAULT_ACCEPT_ROW)).replace("{name}", self_name)
                _rrow = str(mg.get("read_row", _mognet.DEFAULT_READ_ROW))
                _mprompt = _menu(mg.get("menu_prompt", _mognet.DEFAULT_MENU_PROMPT))
                ...
                if _rec_meta:
                    mg_pos[k]["arrive"] = _add_raw(
                        ...
                        _tail_sub)
                mg_pos[k]["read_rows_meta"] = _rows_meta       # pass-through, not text positions
                mg_pos[k]["received_meta"] = _rec_meta
    ...
    savepoint_txids = {}
    for k, pos in sp_pos.items():
        d = {"prompt": mapping[pos[0]], "confirm": mapping[pos[1]]}
        if len(pos) > 2:                                   # the tent row's confirm + no-tents line
            d["tent_prompt"], d["tent_none"] = mapping[pos[2]], mapping[pos[3]]
        savepoint_txids[k] = d
    _passthrough = {"give_to_id", "read_rows_meta", "received_meta"}   # resolved values, not text positions
...
def _emit_scripts(projects, layout, mod_name) -> list:
    ...
    try:
        ...
        raise BuildError(str(ex))
    try:
        _reb = _rebalance.collect(projects)               # the mod's ONE [rebalance] spec (or None)
    except _rebalance.RebalanceError as ex:
        raise BuildError(str(ex))
    try:
