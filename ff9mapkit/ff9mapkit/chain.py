...
def walk(seed_ids, scan_fn, zone_fn, *, forkable_fn=None, max_hops=DEFAULT_MAX_HOPS, zones=None,
         ...
         stop_at_zone_boundary=True) -> GraphResult:
    ...
    ``stop_at_zone_boundary`` (default) the walk stays within the seed's own zone(s). Either way an edge
    rejected SOLELY for being out of scope is recorded as a PORTAL (so you see where the region connects).
    Scripted edges are recorded as SEAMS and not followed unless ``follow_scripted``. ``max_fields`` is a
    hard cap with a loud ``truncated`` flag rather than silently forking the whole game."""
