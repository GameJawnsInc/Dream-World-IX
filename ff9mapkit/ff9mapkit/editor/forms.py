...
FIELD_SPEC = [
    Field("id", "Field ID", INT, "custom field id (>= 4000)"),
    Field("name", "Name", STR, "-> FBG_N<area>_<name> + EVT_<name>.eb"),
    Field("area", "Area", INT, "must be >= 10"),
    Field("text_block", "Text block", OPTINT, "dialogue .mes block (default 1073)"),
    Field("title", "Title", STR, "human label (optional)"),
    Field("borrow_bg", "Borrow BG", STR, "fork: reuse a real field's art (mapid)"),
]
NPC_SPEC = [
    Field("name", "Name", STR, "merge key with the Blender scene"),
    Field("preset", "Preset", PRESET, "vivi / zidane (or a custom model below)"),
    Field("model", "Model id", OPTINT, "custom model (instead of a preset)"),
    Field("animset", "Animset id", OPTINT, "custom animset (with model)"),
    Field("dialogue", "Dialogue", STR, "a line shown when talked to"),
    Field("pos", "Position (x, z)", COORD, "or place the marker in Blender"),
    Field("requires_flag", "Appears when flag set", OPTINT, "story-flag gate"),
    Field("requires_flag_clear", "Appears when flag clear", OPTINT),
]
GATEWAY_SPEC = [
    Field("name", "Name", STR),
    Field("to", "To field id", INT, "destination field"),
    Field("entrance", "Entrance", OPTINT, "arrival entrance (default 0)"),
    Field("zone", "Zone (x z; x z; ...)", ZONE, "4-5 points; or place in Blender"),
    Field("requires_flag", "Opens when flag set", OPTINT),
    Field("requires_flag_clear", "Opens when flag clear", OPTINT),
]
EVENT_SPEC = [
    Field("name", "Name", STR),
    Field("message", "Message", STR, "shown on trigger"),
    Field("give_item", "Give item (id, count)", PAIR),
    Field("gil", "Gil", OPTINT),
    Field("set_flag", "Set flag (idx, val)", PAIR),
    Field("once", "Fire once", BOOL, "off = fires every step you stand in it", default=True),
    Field("zone", "Zone (x z; x z; ...)", ZONE, "4-5 points; or place in Blender"),
    Field("requires_flag", "Fires when flag set", OPTINT),
    Field("requires_flag_clear", "Fires when flag clear", OPTINT),
]
ENCOUNTER_SPEC = [
