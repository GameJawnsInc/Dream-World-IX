...
def _build_zone_choice(tmp_path, build, extra=""):
    ...
    _, _, _, _, ctx, _, _, _, _gw9, _co10, _sp11, _bh12 = build.collect_text(proj)
...
def test_dialogueless_npc_talk_txid_differs_from_choice_prompt(tmp_path):
    ...
    p.write_text(
        ...
        '[[choice.options]]\ntext = "Right"\n', encoding="utf-8")
    proj = build.FieldProject.load(p)
    mes, npc_txids, _e, _cs, ctx, _o, _a, _ch, _gw, _co, _sp, _bh = build.collect_text(proj)
...
def test_zone_choice_pre_choose_disabled_emits_pchm_and_mask(tmp_path):
    ...
    p.write_text(
        ...
        '[[choice.options]]\ntext = "C"\n', encoding="utf-8")
    proj = build.FieldProject.load(p)
    mes, _, _, _, ctx, _, _, _, _gw9, _co10, _sp11, _bh12 = build.collect_text(proj)
...
def test_event_sets_flag_before_message(tmp_path):
    ...
    p.write_text(
        ...
        'message="Found it!"\nset_flag=[8001,1]\n', encoding="utf-8")
    proj = build.FieldProject.load(p)
    _, _, et, _, _, _, _, _, _gw9, _co10, _sp11, _bh12 = build.collect_text(proj)
...
def test_event_received_window_and_space_check(tmp_path):
    ...
    p.write_text(
        ...
        'give_item=[236,1]\nreceived=true\nrequire_space=true\n', encoding="utf-8")
    proj = build.FieldProject.load(p)
    mes, _, et, _, _, _, _, _, _gw9, _co10, _sp11, _bh12 = build.collect_text(proj)
...
def test_conductor_two_actor_field_builds_end_to_end(tmp_path):
    ...
    mes, npc_txids, _ev, cs_txids, _ch, _oe, _ate, _chest, _gw9, _co10, _sp11, _bh12 = build.collect_text(proj)
...
def test_conductor_walk_field_builds_end_to_end(tmp_path):
    ...
    _mes, npc_txids, _e, cs_txids, _c, _o, _a, _ch, _gw, _co10, _sp11, _bh12 = build.collect_text(proj)
...
def _build_synth(tmp_path, body):
    ...
    proj = build.FieldProject.load(p)
    mes, _n, _e, _c, _ch, _o, _a, chest_txids, _gw9, _co10, _sp11, _bh12 = build.collect_text(proj)
...
def test_synth_chest_flag_by_name_resolves(tmp_path):
    ...
    p.write_text(_CHEST_BASE + '[[chest]]\npos = [0, 60]\nitem = ["Potion", 1]\nflag = "treasure_a"\n\n'
                 '[[flag]]\nname = "treasure_a"\nindex = 8720\n', encoding="utf-8")
    proj = build.FieldProject.load(p)
    assert proj.raw["chest"][0]["flag"] == 8720                            # named opened-flag resolved at load
...
def test_synth_gated_chest_has_appearance_gate(tmp_path):
    ...
    _mes, _txids, eb = _build_synth(tmp_path, '[[chest]]\npos = [0, 60]\nitem = ["Potion", 1]\n'
                                    'flag = 8720\nrequires_flag = 8721\n')
    # the guard prefix: push flag 8721 truth, JMP_IFNOT over the 3-byte InitObject
    guard_prefix = region.cond_truthy(region.GLOB_BOOL, 8721) + bytes([region.JMP_FALSE]) + region._i16(3)
