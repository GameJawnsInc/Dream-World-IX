...
def test_collect_text_applies_speaker_and_tail():
    ...
    body, npc_txids, _, _, _, _, _, _ = build.collect_text(_Stub(raw))
...
def test_collect_text_speaker_on_event_and_cutscene():
    ...
    body, _, ev_txids, cs_txids, _, _, _, _ = build.collect_text(_Stub(raw))
...
def test_collect_text_wraps_long_dialogue_but_not_short():
    ...
    body, _, _, _, _, _, _, _ = build.collect_text(_Stub(raw))
...
def test_dialogue_wrap_can_be_disabled(tmp_path):
    ...
    body, _, _, _, _, _, _, _ = build.collect_text(proj)
