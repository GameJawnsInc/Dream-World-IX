...
| File | What it adds |
|---|---|
| `s35-worldmap-model-cull.patch` | **Anti-cull for custom (loose-FBX) overworld MODELS.** A reskinned world actor (`Models/5/<id>/<id>.fbx`) has junk bind-pose `SkinnedMeshRenderer` bounds, and the overworld — unlike the field (`FieldMapActor.ForcedNonCullingMesh`) — has no bounds override, so it frustum-**CULLS**: invisible when the compact standing pose falls outside the stale bounds, visible when running (the spread pose). One line in **`WMWorld.addGameObjectToWMActor`** (the shared choke point both the model-add path and the `SetModel` opcode route through) forces `smr.localBounds` to a `Single.MaxValue` box on every SMR of the attached model, exactly as the field does. Identity-safe: a bundled world model just loses per-actor frustum culling (~1–2 overworld actors, negligible). ★ Diagnosed from the FIRST in-game overworld reskin (2026-07-04 — wide-leg Zidane rendered while running but vanished standing, the textbook culling signature); the fix is IN-GAME UNVERIFIED (awaits the next engine rebuild). → `project-ff9-custom-models`, `project-ff9-worldmap-feasibility`. |
