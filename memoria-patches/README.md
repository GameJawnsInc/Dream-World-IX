...
| File | What it adds |
|---|---|
| `s36-netsync-ghost.patch` | **Exploration co-op "ghost sync"** — two players see each other walk a shared field in real time. New namespace `Memoria.Netsync`: `NetSyncClient` (a `DontDestroyOnLoad` MonoBehaviour bootstrapped from `HonoluluFieldMain.HonoAwake`) + `NetSyncSocket` (TCP host/client, background read/write threads, lock-guarded latest-state slots, staleness timeout), plus one `HonoAwake` line and two csproj `<Compile Include>`s. Reads a `[Netsync]` section from `Memoria.ini` (`Enabled` / `Role`=selftest\|host\|client / `PeerAddress` / `Port` / `TargetField`); **OFF by default** (`Enabled=0` → nothing is created, zero footprint on a normal game). On the target field it spawns the peer as a "ghost" (the `FieldMap.AddPlayer` model recipe minus the controller — renderers force-enabled, huge skinned-mesh bounds so the PSX-shader-projected model isn't frustum-culled, the local player's submesh visibility mirrored) and drives its transform + animation from the peer's `{field,pos,rot,anim}` each frame. NO field/battle/story-state sync (each save is independent). ★ **IN-GAME PROVEN 2026-07-08** — two machines over WiFi, each rendering the other's live Zidane ghost moving + animating. Known cosmetic gap: the field's per-model lighting isn't applied to the ghost. **Also add `<Compile Include="Memoria\Netsync\NetSyncClient.cs" />` + `<Compile Include="Memoria\Netsync\NetSyncSocket.cs" />` to the csproj.** → `project-ff9-multiplayer-injector`. |
