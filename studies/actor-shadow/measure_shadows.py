"""Measure the blob shadows in a rung-0 frame against the CONTROL frame (same bench, every actor shadow = false).

    py studies/actor-shadow/measure_shadows.py <shadows-on run dir> <control run dir> [--shot 1-spawn] [--out PNG]
...
def measure(on: Image.Image, off: Image.Image) -> dict:
    out = {}
...
def sheet(on: Image.Image, off: Image.Image, path: Path, scale: int = 2) -> None:
...
def main(argv=None) -> int:
    ...
    for name, m in measure(on, off).items():
        r = "n/a" if m["ratio"] is None else f"{m['ratio']:.3f}"
        dk = "n/a" if m["darkened"] is None else f"{100 * m['darkened']:.1f}%"
        print(f"{name:7s} floor px {m['floor_px']:6d}   luminance on/control {r}   darkened >=5%: {dk}")
    if a.out:
        sheet(on, off, a.out)
        print(f"wrote {a.out}")
    return 0
