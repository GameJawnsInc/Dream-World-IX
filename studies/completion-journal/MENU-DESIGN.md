...
**The DLL never bakes catalog content.** The kit emits **`JournalPatch.txt`** (JSON) into the mod
folder at deploy — sections (id/title/objective/sc window), entries (predicate/bit/item/gil/th/
...
ImportantItemName/CardName` off the id in the JSON — never a baked string.
...
- **RECORDS** — the remaining aggregate rows (Tetra Master, Party, Combat & Meta) as framed panes.
  No list; the smallest tab.

Missable rendering (LAW 4, unchanged): "PERMANENTLY MISSED" only at `confidence = "owner"`;
`derived` renders "window likely closed"; no verdict text otherwise.
...
- Kit side: `journalcatalog.emit_patch()` -> `JournalPatch.txt`, wired into `deploy_field`/
  `deploy_campaign`'s existing patch-merge lane; a golden test pins the emitted JSON against the
  loaded catalog.
