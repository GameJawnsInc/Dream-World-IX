...
**STATUS: ★ rungs 0 + 0b + T1 + T1b + ROUND 6 ALL OWNER-CONFIRMED in-game. Every DLL-free read path works, the offline `journal report` reads a real save, and the 7-page in-game dashboard opens, holds, and shows its own LIVE numbers on bench 30801. ★ ROUND 6 PLAYTEST-CONFIRMED (2026-08-26): the `[[text_table]]` lane resolves both STRING rows live — T.Hunter rank shows the computed 'G' at 215 points (the old frozen literal was 'H', and a sibling save block at 178 points still reads H, so the letter is per-save computed), Hunt winner resolves 'Vivi' (index 2) — and BOTH cross-validate EXACTLY against the offline `journal report` on the same container. Live-rows-only pages confirmed good (Party 2 lines, Combat & Meta 3, no "S. purchases"). ⚠ Windows self-closing during a playtest = TURBO-DIALOG (F9), not the mod -- check that FIRST (§7.1). ★ ROUND 7 PLAYTEST-CONFIRMED (owner, New Game 2026-08-26): the checklist page's marks update LIVE IN PLAY — two cargo-hold pickups flipped their rows to OK while the three uncollected rows held '--', the names rendered off the live tables in the engine's own item colour, the gil literals and the width were right. The atlas latch join is verified end-to-end (b7174/b7175 verify=playtested). The bench now carries ALL 7 packs of d1.prima-vista (15 menu rows). What round 7 built: the entry catalog is LIVE code — `journalcatalog.py` + shipped `data/journal_catalog.toml` (the 55-section spine + the first authored section, d1.prima-vista, 30 prose-passed rows seeded from the treasure_join atlas; deferrals declared, all 8 laws lint-breakable) and the CHECKLIST surface (`checklist_pages`/`render_checklist`: live `Global.Bit` marks + runtime `[ITEM=]` names + gil literals, packed under the 8-slot/13-line ceilings). ★ ROUND 8 BUILT, UNPLAYTESTED: the NEXT-OBJECTIVE ladder — `[[section]] objective` (authored: prima-vista "Kidnap Princess Garnet"; the rest title-fallback), one `story.next_objective` catalog row (offline read + rank-ladder-class `.eb` expression + generated `objectives` [TBLE], all from ONE `main_story_ladder()`), rendered full-width as the story page's last line; offline cross-validated on 3 real save blocks (SC 1000→"Kidnap Princess Garnet", 6000→"Fossil Roo", 7200→"Alexandria (the coronation)"). AUTHORING SCOPE (owner): through Vivi reaching the roof with Puck — the current 30 rows ARE that scope (castle bits 7223/3793/7211 are past it, next seeds). ★ THE MENU: owner directed a real in-game Journal replacing Folklore's row — design proposal + 4 ratification questions in `MENU-DESIGN.md` (JournalPatch.txt data plumbing, 3 tabs, rungs M0-M5). MENU-DESIGN.md ★ RATIFIED (all 4 as recommended: Folklore hidden / 3 tabs / "????" gating / merged story tab) and rung M0 IS BUILT + DEPLOYED (s81: the Journal takes Folklore's menu seat, skeleton scene, [Journal] Enabled default-on ini gate; gates clean, backup 20260826-152511). M0 ★ OWNER-CONFIRMED (row works, skeleton opens/closes clean) and rung M1 IS BUILT + DEPLOYED (s82: the STORY tab -- NOW banner + the 44-row section list off JournalPatch.txt; the kit now emits the catalog sidecar at every deploy: render_patch/_emit_journal_patch/deploy copy, golden-tested). M1 FIX ROUND 1 (the first cut blanked the screen: ⚠ THE NEWTONSOFT LAW -- the game's Unity-5.2 Mono throws TypeLoadException JITting any method that references the Managed Newtonsoft-13 types, so NO JSON in engine code, ever; JournalPatch.txt is now TAB-separated records; the banner reseated clear of the title plate; per-refresh exception isolation). ★ M2 BUILT ON TOP (same relaunch): the section-detail checklist -- hover a reached section to aim the right pane (live OK/-- marks, runtime names, N/M tally, prose on the help bubble), Confirm descends into it (this is what 'clicking Story' now does), Cancel walks back up. ★ M2 FIX ROUND 1 (owner findings, both closed): (a) THE BEAT LADDER -- the census found the game's own intermediate SC writes inside the opening (1150 shop/fake ticket, 1154 steeple, 1400 play, 1500 chase, 1900 escape, 2005 crash; sc_relative_scan over 818 fields); `[[section.beat]]` folds them into the ONE ladder (banner + offline report + bench .eb, 50 rungs), so the NOW text now tracks the opening beat by beat -- at SC 1000 exactly (pre-shop) no finer signal exists and the act objective stands. (b) THE CURSOR-START LAW -- an armed group with no SetCursorStartSelect lands the finger wherever NGUI defaults and the first Down jumps a page; both lists now seat the cursor before arming (section list on the current row, detail on row 0 + RemoveCursorMemorize). ★ M2 FIX ROUND 2 (owner: beat ladder + cursor-start both CONFIRMED in-game -- 'Follow the boy' appeared after the ticketmaster; Down walks 1-by-1): the CLICK WIRE -- a cell click routes RecycleListItem.OnClick -> itemClicked -> OnRecycleListItemClick -> UIScene.OnListItemClick -> onPress -> OnKeyConfirm, and neither list had subscribed the delegate, so the mouse could hover but never confirm; both lists now wire it at first init. ★ CLICK DESCEND OWNER-CONFIRMED ('fixed') -- M2 CLOSED, all checks green. ★ M3 BUILT, UNPLAYTESTED: the SIDE QUESTS tab -- L1/R1 bumpers page STORY <-> SIDE QUESTS (SFX 1047, the caption rail renames, MuteActiveSound across the rebuild), the 11 side arcs listed by first availability (SC-gated "????" same as story), and the detail pane renders COUNTER rows from the new C records (13 rows: chocobo beak/dig/beaches/paradise, chocographs found/dug, stellazzio, mognet delivered/stiltzkin/central, ragtime, frogs via sysvar, yan) -- the kit names each primitive read (byte/bit/bitcount/field/sysvar), the DLL implements the five kinds once, and a kit test proves each C record equals journal's own offline reader on a synthetic heap. ★ M3 OWNER-CONFIRMED ('looks good'). ★ M4 BUILT, UNPLAYTESTED: the RECORDS tab -- the third bumper page; three always-open groups (Tetra Master / Party / Combat & Meta) feeding the SAME counter pane via 9 new rec.* C records whose kinds call the ENGINE'S OWN statics (QuadMistDatabase.MiniGame_Get*, party.gil, rare_item_obtained.Count, Settings.time h/m-formatted, FieldEntrance u16) -- the aggregate-row equivalent of LAW 5; deviation from the ratified 'framed panes, no list' sketch noted (same pane machinery = smallest new surface; owner may object at playtest). ★ M5 BUILT, UNPLAYTESTED (owner: 'go M5'; same relaunch as M4, backup 20260901-203940): THE MISSABLE COLUMN + polish. (a) LAW 4 rendered: an uncollected row whose window has closed (SC >= close_sc) wears the stock warning RED and its help bubble leads with the verdict -- 'Window likely closed' at derived confidence, 'PERMANENTLY MISSED' only at owner confidence, nothing otherwise; `journalcatalog.missable_verdict` and `JournalUI.MissableVerdict` are the same function and the kit test pins every state; the lint now also refuses a close_sc at/before its section's enter. (b) THE DERIVATION, honest and small: both censuses were regenerated in the fresh worktree and NEITHER gives any of the 30 bits a write-site SC window (the interaction-driven once-flag class PLAN §5 predicted, 257/974 corpus-wide); the one derivable basis is ROOM REACHABILITY off the atlas `fields` -- 24 rows are ALSO written by a disc-3 twin room (1850-1865) and are therefore NOT missable; the 7 single-visit rows (cargo hold 50/51, engine room 57, rooftop 116) are `derived` at the section's own leave (2020); the lone Main-Street-100 row (7215) is unknown and carries NO column (grep cannot prove field 100 is never re-entered). No third-party missable list was consulted -- the selection is ours, from bytes. (c) Polish: a reached section row now shows its own tally ('The Prima Vista & Alexandria   2/30'; counters-only/unauthored rows stay bare); L1/R1 glyphs flank the tab name on the caption rail (FF9UIDataTool.ButtonGameObject draws the LIVE binding -- pad glyph or keyboard key), seated by estimate (label +70, L1 at +20, R1 at rail-right -110) -- a screenshot calibrates; the stale 'JSON' narration in MENU-DESIGN/JournalUI is corrected to TSV. Worktree note: the recycled worktree took the gitignored atlas + census + template cache with it (regenerated), and slot 30801's revert script pointed at the dead worktree's package (repointed by name); the first 'build' silently ran from the wrong cwd behind a `| tail` -- exit codes are now kept in the log. ★ M5 FIX ROUND 1 (owner screenshot, SC 7200 save): the RED rows + 'Window likely closed' bubble CONFIRMED working (LAW 4 render closed). Two removals/fixes: (a) the L1/R1 tab hint is GONE -- the glyphs rendered as empty boxes overlaying the list and the owner ruled it an invented, non-stock idiom (bumper paging stays; discoverability rides the tab caption alone); (b) the per-row tally was clipped at the row edge ('13/30' -> '1', and the pre-existing 'Festival of the Hunt' -> 'the H'), so section-row labels now shrink-to-fit (UILabel.Overflow.ShrinkContent at width 655, the NOW banner's own overflow). Rebuilt `--label s82-journal-m5-fix1`. ★ M5 FIX ROUND 2 (owner screenshot: 'all the text is spilling past the left margin'): the fix-1 shrink-to-fit widened the donor label IN PLACE, and that label is not Left-pivoted, so it grew leftward out of the row. Now pivot-agnostic: capture the BAKED left edge (lp.x - width*pivotOffset.x), re-anchor Left there, alignment Left, width to the row's right margin, ShrinkContent. Rebuilt `--label s82-journal-m5-fix2`, backup 20260901-205742. ★ M5 FIX ROUND 3 (owner: still spilling, and 'don't put the counter on the main selector menu anyway'): the row tally is REMOVED and the donor label is left EXACTLY as baked -- no width/pivot/overflow edits at all; the section list is back to the M4 state the owner confirmed, the N/M tally lives only in the detail header. LESSON: the KeyItem cell's baked label geometry is not ours to reshape from code -- two blind rounds cost two playtests; the long-title clip ('Festival of the Hunt') is a TITLE-budget matter for the catalog, not a label matter for the DLL. Rebuilt `--label s82-journal-m5-fix3`, backup 20260901-213213. NEXT = playtest M4 + M5 fix 3 in ONE relaunch: (M4) bumper twice to RECORDS: Tetra numbers match the Tetra Master dashboard page, gil/key items match the menu proper, play time h/m sane; (M5) on a save PAST the crash (SC >= 2020) hover Prima Vista: the uncollected ship/rooftop rows read RED with 'Window likely closed -- <locator>' in the bubble while Alexandria-town rows stay gray; on the New Game save (SC < 2020) none are red; the story row shows 'N/30'; the L1/R1 glyphs sit clear of the tab name (report the offset if not); then the arc returns to catalog authoring (Evil Forest); also still open from M3: bumper to SIDE QUESTS: available arcs named, unreached ????; hover Chocobo H&C -> beak/dig/beaches numbers match the dashboard's; Stellazzio/Mognet counts match the save; banner reads the save's objective, list shows reached titles + "????" beyond, current row yellow, scroll works, cancel clean) + 30801's story-page objective line (still unplaytested); then M2 = the section-detail pane with live latch marks; the standing §4c owner ask (disc-3/4 checkpointed saves) stays open.**
