r"""TIER W rung 4 -- the gate runner.  `py w4_gates.py` prints X0..X6 with numbers and PASS/FAIL.

X0  NO REGRESSION: r1/r2/r3 (tier-r), w1_gates, w2_gates, w3_gates (if runnable), and EVERY
    tier-r/tier-w test module -- including THIS rung's ``test_reskin.py`` -- still pass, with exact
    counts (tier-r 142 + w1 34 + w2 34 + w3 46 + w4 38 = 294)
...
Reads the user's own install for X1-X3 and X6 (their subject is the REAL ef227 reskin); X0, X4 and
X5 do not need it.  Prints only structure -- offsets, counts, sizes and the small number of stock
scalar VALUES the self-check is already built around (hue/sat/val knobs, entry counts, VRAM cells).
"""
...
def x0_no_regression():
    ...
    try:
        p = subprocess.run([sys.executable, os.path.join(_HERE, "w3_gates.py")],
                           capture_output=True, text=True, cwd=_HERE, timeout=RUNNER_TIMEOUT)
        tail = [l for l in p.stdout.splitlines() if "gates pass" in l]
        got = tail[-1].strip() if tail else "(none)"
        good = p.returncode == 0 and got.startswith("8/8")
        ok = ok and good
        lines.append("%-14s %s   (want %s)" % ("w3_gates.py", got, "8/8"))
    ...
    exact = (counts.get("tier-r") == 142 and counts.get("w1") == 34 and counts.get("w2") == 34
            and counts.get("w3") == 46 and counts.get("w4") == 38 and total == 294)
    lines.append("counts match this rung's own figures (142/34/34/46/38 = 294): %s" % exact)
    ok = ok and exact

    # the readers this rung builds on must be untouched (rescore.py, summon_camera.py,
    # camera_codec.py, ef_container.py, the tier-r tools) -- w3_gates.py's own tracked/untracked
    # split, extended with reskin.py's own new readers.
...
def x1_byte_accounting():
    ...
    return gate("X1 byte accounting (%d bytes, %d targets, against the %d-byte ceiling)"
               % (len(c.changed), len(c.per_target), RS.WHOLE_SET_CEILING), ok, *lines)
