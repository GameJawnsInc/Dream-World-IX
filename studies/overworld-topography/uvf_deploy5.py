"""RUNG F -- THE ROOTS RELIEF RELAX (round 5) -- DEPLOY.

Lands FIXED5 (Y-only position relax of the 2305 synthesized tris' relief, dissolving the
channel/nub/dig-spot anomalies while preserving the crater -- uvf_fix5.py) over the currently-live
FIXED4 bytes at C:/Program Files (x86)/Steam/steamapps/common/FINAL FANTASY IX/FF9CustomMap-world.

Procedure (matches every prior Rung-F deploy in this arc, extended with round 5's geometry-aware live
gate battery since this round moved positions/normals, not just UVs):
  (1) PRE-CHECK: sha256 live Disc1 (180 rung_f-footprint files) vs the staged FIXED4 tree. ABORT on
  ...
  (2) BACKUP: full live rung_f footprint (Disc1 180 + Disc4 180 = 360 files) copied verbatim to
      backups/rungf-relief-predeploy.<timestamp>/ before any write.
  (3) DIFF staged FIXED4 vs staged FIXED5 (180 files each) to find exactly what round 5 changed
      (expected: 15 Terrain files, matching uvf_gates5.py's N_TERRAIN_DIRTY_EXPECTED and the
      falsifier's independently-derived changed-file list).
  ...
  (6) VERIFY sha256 live == staged FIXED5 on all 180 Disc1 files + the changed Disc4 files, and
  ...
  (7) LIVE GATES directly against the live install (not the staged tree): uvf_gates.py's
      ...
      audit -- the geometry-relevant lane this round), gate1_uv_validity (zero-degenerate UV),
      uvf_gates4.py's family-aware one-window-coherence + family mains-rect membership (UV bytes are
      untouched this round but re-measured live, not assumed), and the Sea4-uniformity invariant.

Read-only against the live install except the write in steps 4/5. Writes only the live mod folder
(steps 4/5), a backup copy (step 2), and out/rung_f/uvf_deploy5_report.json. Does NOT touch
...
REPORT_OUT = OUT_DIR / "uvf_deploy5_report.json"
...
BACKUP_DIR = BACKUP_ROOT / f"rungf-relief-predeploy.{TS}"
...
def main():
    assert FIXED4_DIR.exists(), f"missing staged FIXED4: {FIXED4_DIR}"
    assert FIXED5_DIR.exists(), f"missing staged FIXED5: {FIXED5_DIR}"
    assert LIVE_ROOT.exists(), f"missing live install dir: {LIVE_ROOT}"

    report = {"title": "RUNG F -- THE ROOTS RELIEF RELAX (UV-fix round 5) -- DEPLOY REPORT",
              "timestamp": TS,
              "live_mod_folder": str(LIVE_ROOT),
              "source_staged": str(FIXED5_DIR), "base_staged": str(FIXED4_DIR)}

    # -------------------------------------------------------------------------------------------
    # (1) PRE-CHECK -- live Disc1 footprint (180 files) must equal staged FIXED4 exactly
    # -------------------------------------------------------------------------------------------
    log("=" * 100); log("STEP 1 -- PRE-CHECK: live Disc1 vs staged FIXED4"); log("=" * 100)
    fixed4_disc1 = all_footprint_relpaths(FIXED4_DIR, "1")
    mismatches, missing_live = [], []
    for rp in fixed4_disc1:
        lp = LIVE_ROOT / rp
        if not lp.exists():
            missing_live.append(str(rp)); continue
        if lp.suffix == ".txt":
            same = lp.read_bytes() == (FIXED4_DIR / rp).read_bytes()
        else:
            same = sha256(lp) == sha256(FIXED4_DIR / rp)
        if not same:
            mismatches.append(str(rp))
    precheck_ok = (not mismatches) and (not missing_live) and (len(fixed4_disc1) == 180)
    log(f"  fixed4_disc1_files={len(fixed4_disc1)} missing_live={len(missing_live)} mismatches={len(mismatches)} -> {'PROCEED' if precheck_ok else 'ABORT'}")
    report["1_pre_check"] = dict(n_files=len(fixed4_disc1), missing_live=missing_live,
                                  mismatches=mismatches, passed=precheck_ok)
    if not precheck_ok:
        report["status"] = "ABORTED"
        report["abort_reason"] = "live Disc1 does not match staged FIXED4 -- drift or concurrent session"
    ...
    # -------------------------------------------------------------------------------------------
    # (3) DIFF staged FIXED4 vs staged FIXED5 -- find exactly what round 5 changed
    # -------------------------------------------------------------------------------------------
    log("=" * 100); log("STEP 3 -- DIFF staged FIXED4 vs staged FIXED5"); log("=" * 100)
    fixed5_disc1 = all_footprint_relpaths(FIXED5_DIR, "1")
    assert [str(x) for x in fixed5_disc1] == [str(x) for x in fixed4_disc1], "footprint file-set drifted between FIXED4 and FIXED5"
    changed = []
    for rp in fixed4_disc1:
        a = FIXED4_DIR / rp
        b = FIXED5_DIR / rp
    ...
    report["3_diff"] = dict(files_changed=len(changed), list=[str(x) for x in changed],
                             expected=15, matches_expected=(len(changed) == 15))
    ...
    for rp in changed:
        dst = LIVE_ROOT / rp
        dst.write_bytes((FIXED5_DIR / rp).read_bytes())
        log(f"  wrote {rp}")
    ...
    log(f"  disc1_180: bad={len(disc1_bad)}/{len(fixed4_disc1)}")
    ...
    report["6_verify"] = dict(
        disc1_full_180=dict(checked=len(fixed4_disc1), bad=disc1_bad, passed=not disc1_bad),
    ...
    plumb_live = G.plumbing_criteria(LIVE_ROOT, "live_disc1_fixed5")
    ...
    g1_live = G.gate1_uv_validity(FOOTPRINT, LIVE_ROOT, "live_disc1_fixed5")
    ...
    g2_live = G.gate2_sea_plan_disjoint(FOOTPRINT, LIVE_ROOT, "live_disc1_fixed5")
    ...
    wc_live = G4.family_window_coherence_check(LIVE_ROOT, "live_disc1_fixed5", ref)
    ...
    rect_live = G4.family_rect_membership_check(LIVE_ROOT, "live_disc1_fixed5", ref)
    ...
    report["sha_verify"] = ("Disc1 180/180 == FIXED5; Disc4 changed-files == FIXED5 + untouched == "
    ...
    report["notes"] = (
        "The live FF9CustomMap-world install now carries FIXED5: the 2305 synthesized-fill tris' Y-only "
        "relief relax (923 coincident positions / 5557 vertex entries / 2202 tris with a moved vertex / "
        "6606 recomputed geometric-up normals) replaces FIXED4's carried channel/nub/dig-spot relief on "
        "the wedge donut around the crater, while the crater bowl itself (7.92u basin disc, 13 positions) "
        "stays byte-frozen. UVs/tangents/indices/topo untouched (topo remains 0/grass on all synthesized "
        "tris per the standing deliberate deviation). Offline gates (uvf_gates5.py) ALL GREEN, code-disjoint "
        "falsifier CONFIRMED, and the calibration-first per-pixel/height-diff eye (uvf_eye_relief.py) cleared "
        "with anomalies_gone=True and crater_verdict=PRESERVED before this deploy. Playtest 4 pending (owner "
        "to walk the wedge donut at ground level and confirm the channel/nub/dig-spot relief reads as gone "
        "while the crater still reads as a crater); one documented offline residual carried forward -- 12 "
        "non-basin positions whose entire 1-ring is pinned kept relief unreachable by this or any fill-only "
        "round, and a cluster of moves >=0.75u near world (128-148,-1100..-1106) has no local kept-ground "
        "anchor within 12-20u to check against (smooth, passes every invariant, but offline-unfalsifiable in "
        "absolute height) -- see uvf_fix5_falsify.json for the exact site list.")
