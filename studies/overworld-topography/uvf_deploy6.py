"""RUNG F -- THE CARRIED-SPIKE SHAVE (round 6) -- DEPLOY.

Lands FIXED6 (the scoped Y-only shave of the 4 carried-donor dunes-relief spike apexes riding on the
crater's rim, plus their 8 welded fill-weld-neighbors -- uvf_fix6.py) over the currently-live FIXED5
bytes at C:/Program Files (x86)/Steam/steamapps/common/FINAL FANTASY IX/FF9CustomMap-world.

Procedure (matches every prior Rung-F deploy in this arc, base=FIXED5 source=FIXED6):
  (1) PRE-CHECK: sha256 live Disc1 (180 rung_f-footprint files) vs the staged FIXED5 tree. ABORT on
  ...
  (2) BACKUP: full live rung_f footprint (Disc1 180 + Disc4 180 = 360 files) copied verbatim to
      backups/rungf-spikes-predeploy.<timestamp>/ before any write.
  (3) DIFF staged FIXED5 vs staged FIXED6 (180 files each) to find exactly what round 6 changed
      (expected: 3 Terrain files, matching uvf_gates6.py's full-tree sha256 diff and the falsifier's
      independently-derived changed-file list -- Block[1][17], Block[2][17], Block[2][18]).
  ...
  (6) VERIFY sha256 live == staged FIXED6 on all 180 Disc1 files + the changed Disc4 files, and
...
(steps 4/5), a backup copy (step 2), and out/rung_f/uvf_deploy6_report.json. Does NOT touch
...
REPORT_OUT = OUT_DIR / "uvf_deploy6_report.json"
...
BACKUP_DIR = BACKUP_ROOT / f"rungf-spikes-predeploy.{TS}"
...
def main():
    ...
    report = {"title": "RUNG F -- THE CARRIED-SPIKE SHAVE (round 6) -- DEPLOY REPORT",
              "timestamp": TS,
              "live_mod_folder": str(LIVE_ROOT),
              "source_staged": str(FIXED6_DIR), "base_staged": str(FIXED5_DIR)}

    # -------------------------------------------------------------------------------------------
    # (1) PRE-CHECK -- live Disc1 footprint (180 files) must equal staged FIXED5 exactly
    # -------------------------------------------------------------------------------------------
    log("=" * 100); log("STEP 1 -- PRE-CHECK: live Disc1 vs staged FIXED5"); log("=" * 100)
    fixed5_disc1 = all_footprint_relpaths(FIXED5_DIR, "1")
    mismatches, missing_live = [], []
    for rp in fixed5_disc1:
        lp = LIVE_ROOT / rp
        if not lp.exists():
            missing_live.append(str(rp)); continue
        if lp.suffix == ".txt":
            same = lp.read_bytes() == (FIXED5_DIR / rp).read_bytes()
        else:
            same = sha256(lp) == sha256(FIXED5_DIR / rp)
        if not same:
            mismatches.append(str(rp))
    precheck_ok = (not mismatches) and (not missing_live) and (len(fixed5_disc1) == 180)
    log(f"  fixed5_disc1_files={len(fixed5_disc1)} missing_live={len(missing_live)} mismatches={len(mismatches)} -> {'PROCEED' if precheck_ok else 'ABORT'}")
    report["1_pre_check"] = dict(n_files=len(fixed5_disc1), missing_live=missing_live,
                                  mismatches=mismatches, passed=precheck_ok)
    if not precheck_ok:
        report["status"] = "ABORTED"
        report["abort_reason"] = "live Disc1 does not match staged FIXED5 -- drift or concurrent session"
    ...
    # -------------------------------------------------------------------------------------------
    # (3) DIFF staged FIXED5 vs staged FIXED6 -- find exactly what round 6 changed
    # -------------------------------------------------------------------------------------------
    log("=" * 100); log("STEP 3 -- DIFF staged FIXED5 vs staged FIXED6"); log("=" * 100)
    fixed6_disc1 = all_footprint_relpaths(FIXED6_DIR, "1")
    assert [str(x) for x in fixed6_disc1] == [str(x) for x in fixed5_disc1], "footprint file-set drifted between FIXED5 and FIXED6"
    changed = []
    for rp in fixed5_disc1:
        a = FIXED5_DIR / rp
        b = FIXED6_DIR / rp
    ...
    report["3_diff"] = dict(files_changed=len(changed), list=[str(x) for x in changed],
                             expected=3, matches_expected=(len(changed) == 3))
    ...
    for rp in changed:
        dst = LIVE_ROOT / rp
        dst.write_bytes((FIXED6_DIR / rp).read_bytes())
        log(f"  wrote {rp}")
    ...
    log(f"  disc1_180: bad={len(disc1_bad)}/{len(fixed5_disc1)}")
    ...
    report["6_verify"] = dict(
        disc1_full_180=dict(checked=len(fixed5_disc1), bad=disc1_bad, passed=not disc1_bad),
    ...
    plumb_live = G.plumbing_criteria(LIVE_ROOT, "live_disc1_fixed6")
    ...
    g1_live = G.gate1_uv_validity(FOOTPRINT, LIVE_ROOT, "live_disc1_fixed6")
    ...
    g2_live = G.gate2_sea_plan_disjoint(FOOTPRINT, LIVE_ROOT, "live_disc1_fixed6")
    ...
    wc_live = G4.family_window_coherence_check(LIVE_ROOT, "live_disc1_fixed6", ref)
    ...
    rect_live = G4.family_rect_membership_check(LIVE_ROOT, "live_disc1_fixed6", ref)
    ...
    report["sha_verify"] = ("Disc1 180/180 == FIXED6; Disc4 changed-files == FIXED6 + untouched == "
    ...
    report["notes"] = (
        "The live FF9CustomMap-world install now carries FIXED6: the scoped Y-only shave of the 4 "
        "carried-donor dunes-relief spike apexes (topo-41 kept tris) riding 0.72-1.09u above the local "
        "rim reference on the crater mound, welded through to 8 fill positions, 12 positions / 67 vertex "
        "entries total moved, 3 Terrain files changed (Block[1][17], Block[2][17], Block[2][18]). The "
        "crater bowl (7.92u basin disc at world (127.14,-1161.42), floor Y=3.0) is byte-frozen -- 0 "
        "positions, 0 vertex bytes changed inside it -- and the round's own weld/basin/multiset audits "
        "confirm the overall carried-mound height distribution outside the 4 licensed spikes is "
        "untouched. UVs/tangents/indices/topo untouched. Offline gates (uvf_gates6.py) ALL GREEN, a "
        "genuinely code-disjoint falsifier (uvf_fix6_falsify.py, distinct from the build-side script "
        "that happens to share the expected report filename -- see that script's own notes) CONFIRMED "
        "bit-for-bit including its own independent basin-reference-trap reproduction, and the "
        "calibration-first shaded-relief eye (uvf_eye_relief6.py, own from-scratch kept-ground-only "
        "reference fit) cleared with spikes_gone=True and crater_verdict=PRESERVED before this deploy. "
        "Playtest 5 pending (owner to look at the crater mound and confirm the bumpy top is gone while "
        "the crater itself and the sealed seams from playtest 4 are unaffected). One documented "
        "non-gating residual carried forward from round 5's notes: the dunes-lattice-window vs "
        "stock-free-fractional-window texture placement gap (unrelated to this round's geometry claim).")
