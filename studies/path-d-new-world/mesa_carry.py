"""WHOLE-MESA CARRY -- the (15,14) stock mesa carried COMPLETE onto the Disc9 bench.

The endpoint grammar (studies/path-d-new-world/ENDPOINT-GRAMMAR.md) decoded THE TAPER
LAW and named this donor: a pure closed crest ring (zero endpoints), its own ring-1
course and enclosed plateau, boundary = crest weld + ground only. Registration:
studies/path-d-new-world/MESA-CARRY-PREDICTION.md. This builder carries the feature
ENTIRE (wall + crest + ring-1 + plateau + skirt, verbatim verts/uvs/normals/tangents
incl. topograph tags) under the lattice-group pose, and mints ONLY the foot weld --
the level-cut machinery spliced VERBATIM from terrace_wall_strip.py (the code with
two silent playtests behind it), not re-implemented.

THE SKIRT DECISION (the registration's declared freedom, measured 2026-07-31): the
west-skirt continuation in neighbour blk (14,14) is only 4 tris BUT cannot close the
mesh above the cut -- the donor mesa sits on SLOPING donor ground (its ground-weld
line runs y 3.0 east up to 6.8 west). BURY fires for that measured reason: the seat
puts the ENTIRE donor ground-weld line below the bench cut plane, so the level cut
crosses only solid wall faces all the way around. Cost ~3.8u of height (24.4 ->
~20.6u above bench ground), declared.

Regenerate: py -X utf8 mesa_carry.py  (offline gates + renders; --apply to deploy)
"""
...
DONOR_BLK = (15, 14)
SEAT_CLEAR = 0.2                                            # cut-plane clearance above the ground-weld line
...
def main() -> int:
    ...
    y_weld_max = max(bnd_y)
    dy = LOWLAND - (y_weld_max + SEAT_CLEAR)
    ...
    print(f"pose: yaw 0, translate ({tx:+.0f}, {tz:+.0f}) [4u lattice], seat dy "
          f"{dy:+.2f} (ground-weld max y {y_weld_max:.1f} buried below the "
          f"{LOWLAND} cut; the measured skirt decision -- see the module docstring)")
